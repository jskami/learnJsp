...
<body>
		<!-- write_end와 비슷하니까 복붙 후 수정 -->
<%
	request.setCharacterEncoding("utf-8"); // 한글처리를 해줘야 해! 이 녀석은 데이터를 넘기고 받을때 처리 / 상단에 1~2라인은 이 파일 자체를 처리
	String b_num = request.getParameter("b_num"); // update.jsp에서 넘어온 b_num을 받아오기 위해 코딩
...
<%
	...
	try {
		...
		//out.print(conn);
		query = "update tblboard set b_subject = ?, b_name = ?, b_contents = ? where b_num = ?"; // 업데이트니까 업뎃, 셋 구문으로 변경 / 조건은 b_num을 바탕으로 넘겨받은 정보들을 불러오는거야
		...
		pstmt.setInt(4, Integer.parseInt(b_num)); // b_num을 인티저로 형 변환 
