...
import com.it.mapper.Member2Mapper;
...
public class Member2ServiceImpl implements Member2Service {
	...
	@Override
	public Member2VO read(Member2VO member) { // read의 역할 : 특정 글에 대한 값을 받아 읽어준다.
		return mapper.read(member); // member를 넘겨주면 MemberMapper.xml의 read에서 #기호의 번호 하나만 받아서 연산해줄거야
	}
	
	@Override
	public boolean auth(Member2VO member) { // 사용자가 로그인할 때 입력하는 정보는 단 두개(ID,PW)니까 그 두개만 넘어오는거야
		Member2VO tmp = new Member2VO(); // 임시변수를 만들었다.
