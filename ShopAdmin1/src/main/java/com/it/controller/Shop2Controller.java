...
//	private AdminService adminservice;
	...
	public String list(Model model, PageDTO page, HttpSession session) { // 웹에서 처리할 이름(반드시 동일해야 할 필욘 없지만 편의상 동일하게 하는게 좋겠지?), 웹 브라우저로 전달하기 위해 만드는중 / 모델 객체..왜 뜬금 등장?
		...
		model.addAttribute("list", productservice.getList(page)); // getList로 조회한 모든 내용을 list변수로 전달, service.getList()는 다중데이터이다.
		}
		return "/shop2/list";
	}
...
//		product = productservice.read(product);
...
//	}
	...
	@PostMapping("/cart")
	public String cart(HttpSession session, Cartsub2VO cartsub) { // 상품과 개수가 넘어오게 된다. + 세션 정보를 추가한다.
		log.info("=======장바구니 테스트=======");
		log.info(cartsub);
		...
		if (m_id != null) { // 로그인이 되어있을 경우 장바구니 처리 진행
			...
			return "redirect:/shop2/cartinfo"; // redirect: 의 의미는 컨트롤러로 다시 돌아오라는 호출의 의미이다.(컨트롤러를 거쳐서 진행)
		} else { 
			return "redirect:/member2/login"; // 로그인 상태가 아니라면 이렇게 돌려주면 된다.
		}
	}
	...
	public String cartinfo(HttpSession session, Model model) {
		...
		// 세션아이디를 이용해서 cm_code를 조회해야 함 / 로그인이 되어있다는 가정 하에 진행 
			...
			log.info("로그인 상태");
			return "/shop2/cartinfo"; // 현재 페이지로(자기 자신) 이동(반드시 작성) 여기는 redirect를 쓰면 안된다. 이미 위에서 진행 되었기 때문
		} else {
			log.info("로그아웃 상태");
			return "/member2/login"; // 컨트롤러의 메서드를 호출 한 뒤 jsp로 이동, redirect 생략시 jsp페이지로 바로 이동(컨트롤러 안거치고), 컨트롤러 통과 여부를 잘 모르겠으면 redirect를 그냥 쓰는게 낫다.
		}	// 리다이렉트를 하면 다시 또 계산을 하니까 문제가 발생하겠지? 그래서 로그인 하고 페이지 이동할 때 리다이렉트(두 번 일하지말라고)안쓰는거야
		
	}
	
	@PostMapping("/cartupdate")
	public String cartupdate(Cartsub2VO cartsub) { // 현재 위치에서 머무르지 않고 처리한 후 cartinfo로 넘어가면 돼
		cartservice.updateSub(cartsub); // 상세코드와 수량이 넘어가도록 설정
		return "redirect:/shop2/cartinfo";
	}
	
	@GetMapping("/cartdelete")
	public String cartdelete(Cartsub2VO cartsub) { // 삭제 역시 현재 위치에 머무르지 않고 처리하는데.. 일단 테스트니까 보이드로 확인해보자
		cartservice.deleteSub(cartsub);
		return "redirect:/shop2/cartinfo";
	}
	
	@GetMapping("/cartdeleteall")
	public String cartdeleteall(Cartmain2VO cartmain) {
...
//++++++++++++++++++ Order ++++++++++++++++++++//
	...
	public String orderinfo(HttpSession session, Cartmain2VO cartmain, Model model) {
		String m_id = (String)session.getAttribute("m_id");
		String m_name = (String)session.getAttribute("m_name");
		if (m_id != null) {
			
		cartmain.setM_id(m_id);
		Ordermain2VO ordermain = orderservice.orderproc(cartmain); // om_code 획득, orderproc를 호출하여 cm_code 값을 넘김
